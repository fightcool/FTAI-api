...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  // Usage/balance query template applies to any channel type and overrides
  // the built-in balance lookup when present.
  if (formData.usage_query_template?.trim()) {
    try {
      settingsObj.usage_query_template = JSON.parse(
        formData.usage_query_template
      )
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('Failed to parse usage query template:', error)
    }
