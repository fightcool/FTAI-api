  ...
  const overrideFields = (
    ...
    >
      ...
      >
        ...
        <FormField
          control={form.control}
          name='usage_query_template'
          render={({ field }) => (
            <FormItem className='space-y-3 border-t pt-4'>
              <div className='flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between'>
                <div className='space-y-1'>
                  <FormLabel>{t('Usage & Balance Query Template')}</FormLabel>
                  <FormDescription>
                    {t(
                      'Query upstream usage and balance with a custom JSON template; overrides the built-in balance lookup'
                    )}
                  </FormDescription>
                </div>
                <div className='flex flex-wrap gap-2'>
              ...
              <FormControl>
                <JsonCodeEditor
                  ...
                  disabled={sensitiveLocked || isSubmitting}
                  placeholder={t(
                    'Enter a JSON template to query upstream usage and balance'
                  )}
                  heightClassName='h-40 min-h-40 max-h-40'
                />
              </FormControl>
              <FormDescription className='text-xs'>
                ...
                — {t('Channel base URL')}.{' '}
                {t(
                  'mapping fields are dot paths into the response JSON; at least one of balance or remaining is required'
                )}
              </FormDescription>
              <FormMessage />
            </FormItem>
