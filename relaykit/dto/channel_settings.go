...
const (
	// UsageQueryTemplateKind is the only accepted usage_query_template.kind value.
...
type ChannelUsageQueryTemplate struct {
	...
	Mapping UsageQueryMapping `json:"mapping"`
	Unit    string            `json:"unit,omitempty"`
}

// UsageQueryMapping maps balance/usage facts to gjson dot paths. At least one
// of Balance/Remaining is required; Used/Currency/Scope/Available are optional
// display-only facts.
type UsageQueryMapping struct {
...
func (t *ChannelUsageQueryTemplate) Validate() error {
	if t == nil {
		return nil
	}
	if kind := strings.TrimSpace(t.Kind); kind != "" && kind != UsageQueryTemplateKind {
		return fmt.Errorf("usage_query_template.kind must be %q when set", UsageQueryTemplateKind)
	}
	templateURL := strings.TrimSpace(t.URL)
	if templateURL == "" {
		return errors.New("usage_query_template.url is required")
	}
	if !strings.Contains(templateURL, UsageQueryURLPlaceholder) &&
		!strings.HasPrefix(templateURL, "http://") && !strings.HasPrefix(templateURL, "https://") {
		return fmt.Errorf("usage_query_template.url must contain %s or start with http(s)://", UsageQueryURLPlaceholder)
	}
	switch strings.ToUpper(strings.TrimSpace(t.Method)) {
	case "", "GET", "POST":
	default:
		return errors.New("usage_query_template.method must be GET or POST")
	}
	if strings.TrimSpace(t.Mapping.Balance) == "" && strings.TrimSpace(t.Mapping.Remaining) == "" {
		return errors.New("usage_query_template.mapping must set at least one of balance or remaining")
	}
	switch strings.ToUpper(strings.TrimSpace(t.Unit)) {
	case "", "USD", "CNY":
	default:
		return errors.New("usage_query_template.unit must be USD or CNY")
	}
	return nil
}
