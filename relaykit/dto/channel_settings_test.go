...
func TestChannelUsageQueryTemplateValidate(t *testing.T) {
	require.NoError(t, (*ChannelUsageQueryTemplate)(nil).Validate())
	require.NoError(t, (&ChannelUsageQueryTemplate{
		URL:     "{baseUrl}/usage",
		Mapping: UsageQueryMapping{Balance: "balance"},
	}).Validate())
	require.NoError(t, (&ChannelUsageQueryTemplate{
		Kind:   UsageQueryTemplateKind,
		URL:    "https://provider.example/v1/credits",
		Method: "POST",
		Mapping: UsageQueryMapping{
	...
	tests := []struct {
		name     string
		template ChannelUsageQueryTemplate
		want     string
	}{
		{
			name:     "kind mismatch",
			template: ChannelUsageQueryTemplate{Kind: "other", URL: "{baseUrl}/usage", Mapping: UsageQueryMapping{Balance: "balance"}},
			want:     "kind",
		},
		{
			name:     "missing url",
			template: ChannelUsageQueryTemplate{Mapping: UsageQueryMapping{Balance: "balance"}},
			want:     "url is required",
		},
		{
			name:     "url without placeholder or scheme",
			template: ChannelUsageQueryTemplate{URL: "usage", Mapping: UsageQueryMapping{Balance: "balance"}},
			want:     "must contain {baseUrl}",
		},
		{
			name:     "unsupported method",
			template: ChannelUsageQueryTemplate{URL: "{baseUrl}/usage", Method: "PUT", Mapping: UsageQueryMapping{Balance: "balance"}},
			want:     "method must be GET or POST",
		},
		{
			name:     "no balance or remaining mapping",
			template: ChannelUsageQueryTemplate{URL: "{baseUrl}/usage", Mapping: UsageQueryMapping{Used: "used"}},
			want:     "balance or remaining",
		},
		{
			name:     "unsupported unit",
			template: ChannelUsageQueryTemplate{URL: "{baseUrl}/usage", Mapping: UsageQueryMapping{Balance: "balance"}, Unit: "EUR"},
			want:     "unit must be USD or CNY",
		},
	}
